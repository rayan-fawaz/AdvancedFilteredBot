            ...
            try:
                ...
                if ohlcv_response.status_code == 200:
                    ...
                    if ohlcv_data and isinstance(ohlcv_data, list) and len(ohlcv_data) > 0:
                        ...
                        logging.info(f"Successfully calculated ATH price: {ath_price}")
                    else:
                        logging.warning("No valid OHLCV data found in response")
                        ath_price = None
                else:
                    logging.warning(f"OHLCV API returned status code: {ohlcv_response.status_code}")
                    ath_price = None
            except requests.exceptions.RequestException as e:
