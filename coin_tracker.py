...
class CoinTracker:
    ...
    def get_learning_insights(self) -> Dict:
        ...
        # Load already analyzed coins
        analyzed_coins = set()
        try:
            with open('analyzed_coins.json', 'r') as f:
                analyzed_coins = set(json.load(f))
        except:
            pass

        # Get new coins to analyze
        new_coins = {}
        for mint, data in self.tracked_coins.items():
            if mint not in analyzed_coins:
                new_coins[mint] = data
                analyzed_coins.add(mint)

        # Save updated analyzed coins
        with open('analyzed_coins.json', 'w') as f:
            json.dump(list(analyzed_coins), f)

        if not new_coins:
            return {
                "status": "No new coins to analyze",
                "insights": []
            }

        # Analyze prediction patterns
        profitable_predictions = [c for c in new_coins.values() if c['prediction_result'] == "Likely Profitable"]
        high_risk_predictions = [c for c in new_coins.values() if c['prediction_result'] == "High Risk"]
